...
public class SectorDiskImageProvider : IDiskImageProvider, IDisposable
{
    ...
    private const int BYTES_PER_TRACK = 6656; // Standard NIB track size
    ...
    public SectorDiskImageProvider(string filePath)
    {
        ...
        // Open the file with CiderPress2
        ...
        try
        {
            ...
            _diskImage = UnadornedSector.OpenDisk(_stream, new AppHook(null!));
            ...
            if (!_chunkAccess.HasSectors || _chunkAccess.NumTracks != TRACK_COUNT)
            {
                throw new InvalidDataException(
                    $"Expected 35-track 5.25\" disk, got {_chunkAccess.NumTracks} tracks");
            }

            if (_chunkAccess.NumSectorsPerTrack != SECTORS_PER_TRACK)
            {
                throw new InvalidDataException(
                    $"Expected 16 sectors per track, got {_chunkAccess.NumSectorsPerTrack}");
            }
        }
        catch
        {
        ...
        Debug.WriteLine($"Loaded sector disk image: {filePath} ({TRACK_COUNT} tracks, {SECTORS_PER_TRACK} sectors/track)");
    }
    ...
    {
        byte[] trackData = new byte[BYTES_PER_TRACK];
        int offset = 0;
        ...
        for (int sector = 0; sector < SECTORS_PER_TRACK; sector++)
        {
            ...
            catch (Exception ex)
            {
                ...
                Array.Clear(sectorData, 0, BYTES_PER_SECTOR);
            }
            ...
            {
                int remainingSpace = BYTES_PER_TRACK - offset;
                int sectorsLeft = SECTORS_PER_TRACK - sector - 1;
